...
class SumTest {
    ...
    void testReduceSameCurrency(String fromCurrency, BigDecimal fromAmount, String toCurrency, BigDecimal toAmount, BigDecimal expectedAmount) {
    ...
    void testReduceDiffCurrency(String fromCurrency, BigDecimal fromAmount, String toCurrency, BigDecimal toAmount, BigDecimal expectedAmount) {
        testReduce(fromCurrency, fromAmount, toCurrency, toAmount, expectedAmount);
    }

    private void testReduce(String fromCurrency, BigDecimal fromAmount, String toCurrency, BigDecimal toAmount, BigDecimal expectedAmount) {
    ...
    public void testPlusReturnsSum(String baseCurrency, String targetCurrency, BigDecimal amount) {
        ...
        if(baseMoney.getCurrency().equals(targetMoney.getCurrency())) {
            ...
            assertEquals(baseMoney.getAmount().add(targetMoney.getAmount()), money.getAmount());
    ...
    void testPlus(String currency, BigDecimal amount, BigDecimal extraAmount) {
        final BigDecimal expectedAmount = amount.multiply(BigDecimal.valueOf(2)).add(extraAmount);
    ...
    void testTimes(String currency, BigDecimal amount, BigDecimal multiplier) {
        final BigDecimal expectedAmount = amount.add(amount).multiply(multiplier);
        testArithmetic(currency, amount, multiplier, expectedAmount, Expression::times);
    }

    private void testArithmetic(String currency, BigDecimal amount, BigDecimal extraAmount, BigDecimal expectedAmount, BiFunction<Sum, BigDecimal, Expression> operation) {
